...
if ($releases.Count -lt 10) {
    Fail "Expected at least 10 releases, got $($releases.Count)."
}
...
if ($current[0].version -ne "v1.9.0") {
    Fail "Expected current release v1.9.0, got $($current[0].version)."
}
